...
print("=" * 25)
print("1.0.0 | Made by Nikita Meshcheryakov", '\n')
...
def c_tot(update: Update, ctx: CallbackContext) -> None:
    ...
    msg = ctx.bot.send_message(chat_id=update.effective_chat.id, text='\n'.join(message),
                               reply_markup=RED_BLUE_KEYBOARD, parse_mode=ParseMode.MARKDOWN_V2)
...
def c_wyr(update: Update, ctx: CallbackContext) -> None:
    ...
    option1, option2 = escape_markdown(result['option_1'].capitalize(), 2), escape_markdown(
        result['option_2'].capitalize(), 2)
...
def q_buttons(update: Update, ctx: CallbackContext) -> None:
    ...
    query.edit_message_text(text='\n'.join(final_message), reply_markup=RED_BLUE_KEYBOARD,
